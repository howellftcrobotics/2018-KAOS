...
public class KRASH_RobotMecCore
{
    ...
    private enum LAST_SEARCH
    {
        LEFT,
        RIGHT,
        NONE
    } // Used to track mineral search directions
    ...
    public KRASH_RobotMecCore(OpModeConfig opModeConfig)
    {
    ...
    public boolean init()
    {
        ...
        m_opModeConfig.LogInfo("Init motors...");
        try
        {
        ...
        try
        {
            ...
            //Phone in portrait = false, landscape = true.  TRUE IS UNTESTED.  Needs to move to team config
    ...
    public MINERAL_VISION_DIRECTIONS FindMineral(double timeout)
    {
        m_opModeConfig.LogInfo("Entry");

        double xpos;
        double strafePower = 0;
        final double MIN_STRAFE_PWR = 0.25;
        final double MAX_STRAFE_PWR = 0.5;
        final double MAX_ALIGN_LIMIT = 200;
        final double MIN_ALIGNED = 30;
        final int STRAFE_TIME = 200;  //milliseconds
        ...
        {
            m_opModeConfig.LogError("No active GoldAlignDetector for FindMineral()");
            ...
            while (m_opModeConfig.OpMode.opModeIsActive() && (m_elapsedTime.seconds() < timeout))
            {
                if (detector.isFound())
                {
                    ...
                    if (Math.abs(xpos) > MIN_ALIGNED && detector.isFound())
                    {
                        ...
                        TeleOpStrafeDrive(0, 0, strafePower, 1);
                    ...
                    } else
                    {
                        ...
                        if (rightFrontMotor.getCurrentPosition() / MEC_DRIVE_CARDINAL_COUNTS_PER_INCH < -CENTER_RANGE) {
                            return MINERAL_VISION_DIRECTIONS.LEFT;
                        } else if (rightFrontMotor.getCurrentPosition() / MEC_DRIVE_CARDINAL_COUNTS_PER_INCH > CENTER_RANGE) {
                ...
                } else
                {
                    ...
                    switch (lastSearch)
                    {
                        case LEFT:
                            ...
                            StopRobot();
                            double displacement = rightFrontMotor.getCurrentPosition() / MEC_DRIVE_CARDINAL_COUNTS_PER_INCH;
                            ...
                            if (displacement > 2) {
                                ...
                                AutonomousDrive(dispDirection, displacement, MAX_STRAFE_PWR, 2);
                        ...
                        case RIGHT:
                            ...
                            TeleOpStrafeDrive(0, 0, -MAX_STRAFE_PWR, 1);
                            m_opModeConfig.OpMode.sleep(2 * STRAFE_TIME);
