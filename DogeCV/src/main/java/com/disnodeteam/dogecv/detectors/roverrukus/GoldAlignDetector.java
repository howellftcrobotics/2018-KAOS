...
public class GoldAlignDetector extends DogeCVDetector {
    ...
    public double yPosLimit       = 325;  // Detector y location must be greater then this.  Value between 0 and 500.
    ...
    public boolean rotateMat = false; //Phone in portrait = false, landscape = true
    ...
    public Mat process(Mat input) {
        ...
        //Following has not been tested.  Could have unexpected results.
        if(rotateMat){
        ...
        for(MatOfPoint cont : contoursYellow){
            ...
            if(score < bestDiffrence && (rect.y + (rect.height / 2) > yPosLimit)){
                bestDiffrence = score;
                bestRect = rect;
            }
        }
        ...
        if(bestRect != null){
            ...
            //Imgproc.putText(displayMat,"Current X: " + yPos,new Point(10,getAdjustedSize().height - 10),0,0.5, new Scalar(255,255,255),1);
